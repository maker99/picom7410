...
# Documentation from http://www.plicht.de/ci-v/
...
def itobcd(i):
    i = i.zfill(9)
    out = r'\x'+i[8]+'2'+r'\x'+i[6]+i[7]+r'\x' + \
        i[4]+i[5]+r'\x'+i[2]+i[3]+r'\x'+i[0]+i[1]
    return out
...
def ic7410_set_mode_filter(mode, filter):
    ...
    s = ser.write("\xfe\xfe\x80\x00\x01" + mode.decode('string_escape') +
                  filter.decode('string_escape') + "\xfd")


def ic7410_set_mode_filter_2(mode, filter):
    mode = r'\x0' + str(mode)
    filter = r'\x0' + str(filter)
    s = ser.write("\xfe\xfe\x80\x00\x06" + mode.decode('string_escape') +
                  filter.decode('string_escape') + "\xfd")
...
def ic7410_set_vfo_mode2(vfo):
    vfo = r'\x0' + str(vfo)
    s = ser.write("\xfe\xfe\x80\x00\x07" +
                  vfo.decode('string_escape') + "\xfd")
...
def ic7410_set_memory_channel(channel):
    ...
    s = ser.write("\xfe\xfe\x80\x00\x08\x00" +
                  channel.decode('string_escape') + "\xfd")
...
def ic7410_show_frequency(cmd):
    ...
    print("Frequency:" + cmd[10:11]+"."+cmd[11:12]+cmd[8:9]+cmd[9:10] +
          "." + cmd[6:7] + cmd[7:8] + cmd[4:5] + "." + cmd[5:6] + cmd[2:3])


def ic7410_show_band_edge(cmd):
    cmd = ''.join(cmd).encode('hex')
    print("Band Start:" + cmd[10:11]+"."+cmd[11:12]+cmd[8:9]+cmd[9:10] +
          "." + cmd[6:7] + cmd[7:8] + cmd[4:5] + "." + cmd[5:6] + cmd[2:3])
    print("Band End:" + cmd[22:23]+"."+cmd[23:24]+cmd[20:21]+cmd[21:22] + "." +
          cmd[18:19] + cmd[19:20] + cmd[16:17] + "." + cmd[17:18] + cmd[14:15])
...
def ic7410_read():
    ...
    while s != "\xfe":
        if debug:
            print("waiting for sync...." + s.encode('hex'))
        s = ser.read()
    if ser.read() == "\xfe":
        if debug:
        ...
        while s != "\xfd":
            ...
            if i == 0:
                if debug:
                    print("TO:" + s.encode('hex'))
            elif i == 1:
                if debug:
                    print("FROM:" + s.encode('hex'))
            else:
                command.append(s)
            i += 1
        command.pop()
    ...
    if len(cmd) > 2:
        if cmd[0] == "\x00":
            ...
            print("Unknown Command (" +
                  cmd[0].encode('hex') + "), mail blendz@shaw.ca")
            print(cmd)
ser.close()
